...
* 你知道默认 simulator 是 VCS，NC/Incisive 是完整备选 simulator；
* 你能在 shell 中运行 ``pwd``、``ls``、``rg`` 和 ``make help``。

学完本章你能：

1. 区分 DUT、TB、env、agent、monitor、scoreboard、coverage、sign-off 和 LEC；
2. 看到 ``build/smoke_vcs``、``build/smoke_nc``、``cov.vdb``、``cov_work`` 时知道它们
   分别属于哪条工具路径；
3. 解释最新 demo 数据中的 LINE 95.05%、GROUP 69.42%、OVERALL 65.17% 和
...
软件世界可能指日志，在本平台中通常指 retire trace transaction；``coverage`` 既可能是
VCS 的 LINE/BRANCH/TOGGLE，也可能是 SystemVerilog covergroup 的 GROUP；``signoff``
...
* 把 ``make smoke`` 的 PASS 当作 sign-off PASS，忽略 directed、riscv-dv、formal、
  compliance 和 LEC；
* 把 ``GROUP`` 当作 URG 综合分，或者把 LINE 95.05% 当成功能覆盖率；
* 把 NC 误认为只能看波形，忽略它在 v2 基线中同样支持 compile / smoke / regress /
...
VCS 与 NC 都能跑 EH2-Veri，但默认职责不同。当前口径是：VCS 为默认 simulator 和
主线，NC/Incisive 为完整备选 simulator，可用于 compile、smoke、regress、signoff、
demo 和波形调试。不要再使用“NC 只能看波形”的旧说法。
...
.. list-table:: VCS / NC 对照自测
   ...
   * - scope 配置
     - ``cover.cfg``，``+tree core_eh2_tb_top.dut``
     - ``cov_full_nc.ccf``，选择 DUT-only instance
   * - 波形
     - FSDB，通常用 Verdi 打开
...
4. VCS coverage 使用 ``-cm line+tgl+assert+fsm+branch``，不包含 ``cond``。答案：对。
5. NC 的 ``cov_full_nc.ccf`` 是 v2 基线必须覆盖的配置资产。答案：对。

§5  覆盖率数字题
----------------

最新 demo 的 coverage dashboard 使用统一展示口径：
