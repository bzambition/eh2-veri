...
§17  v2-19 ``eh2_fcov_if.sv`` 全文段落级精读
--------------------------------------------------------------------------------

``eh2_fcov_if.sv`` 是非 PMP 微架构 functional coverage 的主接口。此前章节已经
解释了主要 covergroup，但 literalinclude 仍只覆盖中间片段。v2-19 补齐全文，保证
接口端口、分类函数、每个 covergroup、enable plusarg 和采样实例都能被行级审计。

.. literalinclude:: ../../../../dv/uvm/core_eh2/fcov/eh2_fcov_if.sv
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/core_eh2/fcov/eh2_fcov_if.sv:全文

逐段精读：

* L1-L12：文件头和 `include。``eh2_def.sv`` 提供 decode packet、predict packet 等
  typedef，是后续 coverpoint 读取 ``i0_dec``、``i1_dec`` 字段的基础。
* L13-L101：interface 端口列表。输入按流水线 valid、decode packet、branch/flush、
  exception、interrupt、debug、LSU、IFU PMU、commit 和 clock/reset 分组，全部来自
  ``core_eh2_tb_top.sv`` 对 DUT hierarchy 的连接。
* L102-L120：``instr_category_e`` 枚举。该枚举把 ALU、MUL、DIV、branch、jump、
  load/store、CSR、EBREAK/ECALL/MRET、fence、atomic、illegal 和 none 统一成覆盖分类。
* L121-L143：``get_i0_instr_category``。函数先检查 I0 valid，再按 legality、trap 类、
  fence、branch、CSR、load/store、mul/div、atomic、jalr 顺序归类，避免一个指令被多个
  coverpoint 分散解释。
* L144-L166：``get_i1_instr_category``。I1 与 I0 逻辑对称，但以 ``dec_ib1_valid_d`` 和
  ``i1_dec`` 为输入，用来覆盖 dual issue 的第二 slot。
* L167-L191：stall 类型枚举与 ``get_stall_type``。load/store/AMO/decode/presync/
  postsync/fetch stall 被收敛为一个分类，后续 cross 用它解释 pipeline bubble 来源。
* L192-L211：flush、interrupt、debug 状态枚举和 ``uarch_cg`` 声明。采样点绑定
  ``posedge clk_i``，reset gate 由 covergroup 内各 coverpoint/iff 控制。
* L212-L416：``uarch_cg``。该主 covergroup 覆盖 I0/I1 指令类别、stall、branch taken/
  mispredict、flush、exception、LSU exception、interrupt、debug、dual issue、
  compressed、I-cache、LSU external/misaligned 以及关键 cross。
* L421-L432：``csr_cg``。该组只看基础 CSR 操作类型，帮助判断 CSR read/write/set/clear
  是否都被随机或 directed 测试触发。
* L437-L468：``dual_issue_cg``。该组分别分类 I0/I1 指令，并对两个 slot 做 cross，
  是 EH2 双发射区别于单 issue Ibex 的关键 coverage 资产。
* L473-L494：``interrupt_cg``。外部、timer、software、NMI、correctable-error interrupt
  与 debug mode 做组合，解释中断进入和 debug 状态的交界。
* L501-L557：``csr_warl_cg``。该组把标准 M-mode CSR、EH2 custom CSR、PIC CSR、debug
  CSR 与 CSR 操作类型交叉，用于检查 WARL/自定义 CSR 流量是否足够。
* L563-L654：``instr_detail_cg``。branch subtype、load/store width、ALU subtype、
  signed mul/div、mul/div/rem、sync 类型、compressed 宽度和 I0 类别 cross 都在这里。
* L664-L715：``controller_fsm_cg``。该组覆盖 debug running/halted/active、exception
  entry、interrupt entry、MRET、flush reason 及相关 cross，用于解释 controller 状态覆盖。
* L721-L764：``pipeline_state_cg``。该组覆盖 pipe utilization、E4 commit、stall、
  branch mispredict 及 cross，直接服务 dual issue 与 pipeline occupancy 的覆盖判断。
* L769-L797：``fcov_en`` plusarg 和 covergroup 实例化。``+enable_eh2_fcov`` 打开采样；
  未打开时接口仍编译和连接，但不创建无意义覆盖样本。

§18  v2-56 ``eh2_cov_waiver_pkg.sv`` 全文行段级精读
--------------------------------------------------------------------------------

``dv/uvm/core_eh2/fcov/cov_waivers/eh2_cov_waiver_pkg.sv`` 是 functional coverage waiver 的
SystemVerilog runtime helper。它把 YAML-like waiver 文件解析为 associative array，供 testbench
或 coverage 报告路径按 ``coverage_point`` 查询 active waiver。该 package 是轻量运行时辅助层，
不是完整 YAML parser，也不替代 release sign-off 中的 waiver 审查。

.. literalinclude:: ../../../../dv/uvm/core_eh2/fcov/cov_waivers/eh2_cov_waiver_pkg.sv
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/core_eh2/fcov/cov_waivers/eh2_cov_waiver_pkg.sv:全文

逐段精读：

* L1-L12：文件头说明 package 目的、waiver 语义和使用方式。waiver 用来记录 known-unreachable 或
  architecturally blocked coverage point，典型调用顺序是 import package、load waiver，再用
  ``is_waived`` 判断某个 covergroup/cross 是否被排除。
* L13-L32：package、结构体和全局存储。``cov_waiver_t`` 保存 name、coverage point、reason、
  author、date、ticket 和 status；``waivers[string]`` 以 coverage point 为 key 做 associative
  lookup，因此查询复杂度接近 O(1)，也要求 coverage point 字符串稳定。
* L34-L60：``load_waivers`` 目录入口占位。SystemVerilog 不能可移植地枚举目录，所以这里的
  ``$fopen({dir, "/"}, "r")`` 只是占位实现，函数实际提示调用方逐个使用 ``load_waiver_file``，
  或通过 filelist wrapper 驱动加载。
* L62-L145：``load_waiver_file`` 单文件解析器。函数打开 YAML-like 文件，初始化 waiver struct，
  逐行 trim，跳过空行和注释，遇到 ``waiver:`` 后解析 ``key: value``，支持去掉双引号，并把
  folded scalar 指示符 ``>`` 清成空字符串；最后只存入 ``coverage_point`` 非空且 ``status`` 为
  ``active`` 的 waiver。
* L147-L172：``load_waiver_filelist`` 批量入口。该函数读取一行一个路径的文本 filelist，跳过空行和
  ``#`` 注释，再逐项调用 ``load_waiver_file``。这规避了 SV 目录枚举不可移植的问题，也让 CI 或
  runner 可以显式控制加载顺序和文件集合。
* L174-L201：查询 API。``is_waived`` 只返回 key 是否存在；``get_waiver`` 返回完整 struct，
  找不到时返回字段清空的 empty struct。调用方可以先用 bit API 做快路径，再在报告中取 reason、
  author 和 date。
* L203-L224：``print_waivers`` 诊断输出。函数打印当前 associative array 中所有 active waiver；
  若数量为 0，会显式输出 ``(none)``。这适合在仿真 log 中确认 waiver 文件是否真的被加载。
* L226-L247：``str_trim`` 字符串清理。函数手写前后空白剥离，处理 space、tab、newline 和
  carriage return，返回原字符串的 ``substr``。这是前面解析器避免空白扰动 key/value 的基础。
* L249-L256：``str_find`` 查找第一个字符位置。解析器只需要寻找冒号，因此 helper 接收 ``ch``，
  逐字符比较 ``s[i]`` 与 ``ch[0]``，找不到返回 -1。
* L258：``endpackage`` 结束 package，关闭 waiver helper 的命名空间。

接口关系：

* 被调用：coverage/waiver runtime 需要加载 waiver 时 import 该 package，并调用
  ``load_waiver_file`` 或 ``load_waiver_filelist``。
* 调用：只使用 SystemVerilog 文件 I/O、字符串 API 和 ``$display``，不依赖 DPI 或外部 YAML 库。
* 共享状态：YAML 文件中的 ``coverage_point`` 必须与 ``eh2_fcov_if.sv`` 或 ``eh2_pmp_fcov_if.sv``
  中的 covergroup/cross 名称一致；``status`` 只有 ``active`` 会进入运行时 store。

解析边界：

* 该 parser 只解析简单 ``key: value`` 行；当 ``reason: >`` 这样的 folded scalar 出现时，它会把
  ``reason`` 暂时置空，而不会继续拼接后续缩进行。
* 这种实现适合 runtime waiver lookup 和 log 诊断，但不适合作为 release YAML schema validator。
  正式 sign-off 仍应由 Python/CI gate 校验 author、date、ticket、expiry 或 active 状态。
