...
§16  v2-32 riscv-dv 配置与 testlist 全文行段级精读
--------------------------------------------------------------------------------

本节补齐 riscv-dv 扩展目录中仍未达到全文 ``literalinclude`` 覆盖的配置类资产。
前文已经用摘录解释了 core setting、template、hook、testlist 和 CSR 描述；这里把
完整文件纳入 Sphinx 渲染，并按连续行段说明它们如何约束生成器、UVM test 和 cosim。

§16.1  ``riscv_core_setting.sv`` — 当前渲染后的 EH2 setting
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/riscv_dv_extension/riscv_core_setting.sv
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/core_eh2/riscv_dv_extension/riscv_core_setting.sv:全文

逐段精读：

* L1-L10：文件头和基础参数把目标定义为 RV32 integer core：``XLEN=32``、
  ``NUM_GPR=32``，无 floating-point GPR 和 vector GPR。这是 riscv-dv 随机寄存器、
  operand 宽度和指令合法性的根输入。
* L12-L18：vector 参数保留默认结构值，但 ``VECTOR_EXTENSION_ENABLE=0``，表示当前
  EH2 target 不启用 vector 指令。保留这些参数是为了满足 riscv-dv target setting 的
  schema，而不是声明 DUT 支持 vector。
* L20-L27：``NUM_HARTS=1``、``SATP_MODE=BARE``、``supported_privileged_mode``
  只有 ``MACHINE_MODE``，并开启 unaligned load/store 生成。该段把 EH2 验证目标限制在
  machine-only、bare memory model。
* L29-L39：``supported_isa`` 声明 RV32I/M/A/C 和 Zba/Zbb/Zbc/Zbs。生成器据此知道哪些
  instruction group 可以进入随机池；具体 testlist 仍可用 ``gen_opts`` 调整强度或定向 stream。
* L41-L48：interrupt mode 支持 direct/vectored，最多 32 个 vector；PMP/ePMP 默认关闭，
  debug mode 打开，U-mode trap 和 sfence 关闭。PMP 专项测试会通过 testlist plusarg
  单独打开 PMP 生成，不等同于这里的默认 setting。
* L50-L56：kernel section 参数满足 riscv-dv machine-only 流程的结构需要。``num_of_kernel_data_pages=0``
  表示不额外生成 kernel data page，但 stack/program instruction count 仍有默认值。
* L58-L98：``implemented_csr`` 列出 riscv-dv 已有枚举名的标准 CSR，覆盖 ID、status、
  trap、counter/HPM、debug 和 trigger CSR。该白名单决定 CSR test 可以选择哪些符号 CSR。
* L100-L132：``custom_csr`` 以 12-bit 地址列出 EH2/VeeR 自定义 CSR，包括 ``mscause``、
  region/cache/debug/timer/ECC/PIC 相关地址。使用数值是因为 upstream riscv-dv 没有这些
  专有 CSR 的枚举名。
* L134-L152：interrupt 与 exception coverage 列表包含 machine software/timer/external
  interrupt，以及 instruction access fault、illegal、breakpoint、load/store fault、
  misaligned 和 M-mode ecall。这些枚举与 ``csr_description.yaml`` 的 trap CSR 字段共同
  限定生成和覆盖边界。

接口关系：

* 被调用：riscv-dv custom target 读取该文件，作为 EH2 生成器 setting。
* 调用：该文件本身不调用函数；它提供参数、数组和常量。
* 共享状态：``supported_isa``、CSR 白名单和 interrupt/exception 枚举必须与 UVM testlist、
  ``csr_description.yaml`` 和 directed stream 保持一致。

§16.2  ``riscv_core_setting.tpl.sv`` — profile 可渲染模板
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/riscv_dv_extension/riscv_core_setting.tpl.sv
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/core_eh2/riscv_dv_extension/riscv_core_setting.tpl.sv:全文

逐段精读：

* L1-L18：模板头与固定参数和渲染后文件相同，仍声明 RV32 integer target、关闭 vector
  extension，并保留 riscv-dv 需要的 vector 参数占位。
* L20-L27：``NUM_HARTS`` 使用 ``{{ NUM_THREADS }}`` 模板变量，由 EH2 profile 渲染；
  其余 privileged mode、unsupported instruction 和 unaligned load/store 设置保持固定。
* L29-L49：``supported_isa`` 中 RV32A 和四个 bitmanip group 被 ``//% if`` 条件包围。
  ``ATOMIC_ENABLE``、``BITMANIP_ZBA/ZBB/ZBC/ZBS`` 由 ``eh2_configs.yaml`` 参数控制，
  因而同一模板可生成 minimal 或 full feature setting。
* L51-L58：interrupt/debug/PMP 默认设置与渲染后文件一致。模板没有把 PMP/ePMP 做成
  profile 参数，说明当前 profile 渲染主要控制 hart 数、atomic 和 bitmanip ISA group。
* L60-L66：kernel section 参数固定，保证不同 profile 生成出的 setting 仍满足 riscv-dv
  target schema。
* L68-L108：标准 CSR 白名单固定，不随 profile 条件变化。即使 minimal 关闭某些 feature，
  CSR 生成边界仍由该数组和 testlist option 共同约束。
* L110-L142：EH2 custom CSR 地址表固定，覆盖 secondary cause、cache/memory control、
  timer、ECC 和 PIC CSR。它必须与 CSR YAML 和 RTL CSR map 一起维护。
* L144-L162：implemented interrupt/exception 列表固定。该段让 coverage/CSR 生成知道
  EH2 machine-only target 应覆盖哪些 trap cause。

接口关系：

* 被调用：``render_config_template.py`` 或 Makefile ``core_config`` target 读取该模板并输出
  ``riscv_core_setting.sv``。
* 调用：模板不直接执行逻辑；条件块由渲染器解释。
* 共享状态：模板变量来自 ``eh2_configs.yaml`` 的 ``parameters``，输出路径由 staged flow
  决定。

§16.3  ``user_extension.svh`` — EH2 生成扩展入口
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/riscv_dv_extension/user_extension.svh
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/core_eh2/riscv_dv_extension/user_extension.svh:全文

逐段精读：

* L1-L5：文件头说明这是 riscv-dv user extension hook，用于注册 EH2-specific class
  override 和 directed instruction stream。它本身很短，但位于 generator include 链的入口。
* L7-L8：两个 include 分别引入 ``eh2_asm_program_gen.sv`` 和
  ``eh2_directed_instr_lib.sv``。前者覆盖程序框架、debug/NMI/mailbox 生成；后者提供
  CSR、PIC、debug、atomic、exception 和 hazard directed stream。

接口关系：

* 被调用：riscv-dv custom target include 该 hook。
* 调用：通过 SystemVerilog preprocessor include 两个 EH2 扩展源文件。
* 共享状态：testlist 中 ``+directed_instr_0=eh2_*`` 的类名必须来自这里包含的 directed
  instruction library。

§16.4  ``testlist.yaml`` — riscv-dv 主随机/定向回归清单
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/riscv_dv_extension/testlist.yaml
   :language: yaml
   :linenos:
   :caption: dv/uvm/core_eh2/riscv_dv_extension/testlist.yaml:全文

逐段精读：

* L1-L37：基础 arithmetic、random、jump 和 CSR test 设定 ``gen_test``、
  ``gen_opts``、``rtl_test`` 与 ``iterations``。``riscv_csr_test`` 显式关闭 cosim 并
  ``skip_in_signoff``，说明 CSR 随机写读仍存在不适合 release lockstep 的边界。
* L38-L70：load/store、unaligned、mul/div 和 bitmanip 入口覆盖内存访问、非对齐访问、
  乘除法分布和 Zb* proof。bitmanip 项使用 ``cosim_bitmanip.S`` 作为 directed source，
  因而它是 testlist 中从 riscv-dv 随机项切到手写 assembly 的例子。
* L72-L105：bitmanip full/balanced/OT EarlGrey 和 atomic test 继续复用 bounded
  directed assembly。多个强度名称映射到同一源码，差异主要体现在 iterations 和 sign-off
  解释口径。
* L106-L149：interrupt、single IRQ、debug 和 debug CSR test 通过 ``gen_opts`` 和
  ``sim_opts`` 连接 riscv-dv generator 与 UVM sideband sequence。``eh2_pic_int_stream``
  和 ``eh2_debug_csr_stream`` 名称依赖 ``user_extension.svh`` include 的 directed library。
* L150-L202：stress、dual issue、exception、breakpoint、CSR hazard 和 exception stream
  test 覆盖 pipeline 与 trap 压力。``riscv_csr_hazard_test`` 关闭 cosim 并跳过 signoff，
  说明 hazard stream 用于 RTL 专项观察，而不直接作为 release proof。
* L203-L226：PMP basic/disable/random test 通过 ``+enable_pmp``、region 数和 granularity
  控制 generator，并使用专门的 UVM PMP test class。
* L227-L264：PC/RF/RAM/ICache/memory integrity、reset 和 single-step test 覆盖 fault
  injection 与 debug 单步。integrity 项标记 ``cosim: rtl_only``，表明它们的期望行为不完全
  适合 Spike architectural lockstep。
* L265-L288：ePMP MML/MMWP/RLB test 使用 ``+enable_pmp`` 和 8 个 region，连接 ePMP
  UVM test class。这里不在 core setting 中全局打开 ePMP，而是由具体测试驱动。
* L289-L336：memory error 与 issue 61 integrity fault injection 继续使用 base generator，
  但通过不同 ``rtl_test`` 触发 PC、RF、RAM、ICache 或 generic memory integrity 路径。
* L338-L455：debug/IRQ 组合项覆盖 WFI、CSR 中断、嵌套 IRQ、debug-in-IRQ、DRET、
  ebreakm/u 和单 debug pulse。它们用 ``sim_opts`` 打开 ``enable_irq_seq``、
  ``enable_debug_seq`` 或 ``enable_debug_single``，因此 testlist 是 sideband agent 的
  调度入口。
* L456-L480：invalid CSR、fetch enable check 和 jump stress test 覆盖 illegal CSR、
  fetch gating 和 back-to-back jump。``riscv_jump_stress_test`` 显式 ``cosim: enabled``，
  用于把 branch/jump 压力纳入 lockstep。
* L482-L532：debug trigger、debug stress、debug branch/jump 和 debug CSR entry test
  细化 issue 54 debug proof。``eh2_breakpoint_stream`` 再次证明 directed stream 名称
  必须与 EH2 extension include 对齐。
* L534-L555：combined trap/interrupt/debug stress 和 PMP out-of-bounds test 是末段
  高强度 sign-off 入口。前者混合 interrupt、illegal、ebreak 和 unaligned；后者使用
  load/store random stream 和 50 次 iteration 拉高 PMP 边界覆盖。

接口关系：

* 被调用：metadata 和 riscv-dv runner 读取该 YAML，按 ``test`` 名称、``gen_test``、
  ``rtl_test``、``iterations`` 和 sign-off 标记生成回归任务。
* 调用：YAML 不执行函数；它通过 ``gen_opts`` 影响 generator，通过 ``sim_opts`` 影响 UVM
  sideband sequence，通过 ``test_srcs`` 选择手写 assembly。
* 共享状态：``rtl_test`` 必须存在于 UVM test library；``directed_instr`` 名称必须由
  ``user_extension.svh`` 引入；``cosim`` 和 ``skip_in_signoff`` 会被 sign-off gate 消费。

§16.5  ``csr_description.yaml`` — CSR 字段模型
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/riscv_dv_extension/csr_description.yaml
   :language: yaml
   :linenos:
   :caption: dv/uvm/core_eh2/riscv_dv_extension/csr_description.yaml:全文

逐段精读：

* L1-L7：文件头声明该 YAML 描述 riscv-dv CSR test generation 使用的 CSR 字段，字段格式
  使用 ``[msb:lsb]`` 或 ``[bit]``，类型包括 R/W、RO、WARL、WLRL 等。
* L8-L39：M-mode 基础 CSR 从 ``mstatus`` 和 ``misa`` 开始。``mstatus`` 只描述 MIE、
  MPIE 和 MPP，``misa`` 描述 MXL 与 extension bit，限定生成器可理解的字段粒度。
* L41-L81：``mie`` 和 ``mtvec`` 描述 interrupt enable 与 trap vector。``mie`` 暴露
  software/timer/external/internal timer/correctable error enable，``mtvec`` 把 base 和
  mode 分成 WARL 字段。
* L83-L120：``mcountinhibit``、``mscratch`` 和 ``mepc`` 覆盖 counter inhibit、scratch
  和 exception PC。``mepc`` bit 0 写成 RO，体现 32-bit compressed/对齐语义下的 WARL 边界。
* L121-L169：``mcause``、``mtval`` 和 ``mip`` 形成 trap 状态三件套。``mip`` 只读 pending
  位与 ``mie`` 的 enable 位一一对应，用于 interrupt CSR 生成和检查。
* L171-L205：ID CSR ``mvendorid``、``marchid``、``mimpid`` 和 ``mhartid`` 均为 RO
  32-bit 字段。它们进入描述表但不应被随机写期望改变。
* L206-L239：``mcycle/mcycleh/minstret/minstreth`` 是 32-bit counter low/high half，
  类型为 R/W。该段与 core setting 的 counter CSR 白名单一致。
* L241-L279：debug CSR ``dcsr`` 和 ``dpc`` 描述 debug spec version、ebreakm、single-step、
  debug cause、NMI pending 和 privilege 字段。debug testlist 和 debug stream 会依赖这些字段。
* L289-L315：trigger CSR ``mtsel``、``mtdata1`` 和 ``mtdata2`` 都以 32-bit R/W 字段描述，
  为 breakpoint/trigger directed stream 提供 CSR 名称和地址。
* L316-L358：WD/Microchip custom CSR 包括 ``mscause``、``mrac``、``mfdc``、``mcgc`` 和
  ``mpmc``。``mpmc`` 类型为 ``R0W1``，提醒读者该文件允许表达非普通 R/W 的写语义。
* L359-L408：internal timer CSR 描述 ``mitcnt0/1``、``mitb0/1`` 和 ``mitctl0/1``，
  地址从 ``0x7D2`` 到 ``0x7D7``，均为 32-bit R/W。
* L410-L435：ECC/error CSR ``micect``、``miccmect`` 和 ``mdccmect`` 分别对应 ICache、
  ICCM 和 DCCM ECC error count threshold。
* L437-L487：PIC CSR 描述 ``meivt``、``meihap``、``meipt``、``meicpct``、
  ``meicurpl`` 和 ``meicidpl``。其中 ``meivt.base`` 使用 ``[31:10]``，PIC priority
  字段多为 4-bit，必须与 ``eh2_pic_int_stream`` 中的硬编码地址保持一致。

接口关系：

* 被调用：CSR generator、coverage 和 EH2 directed stream 使用该 YAML 的地址和字段信息。
* 调用：YAML 不调用代码。
* 共享状态：CSR 地址必须与 ``riscv_core_setting.sv`` 的 ``implemented_csr``/
  ``custom_csr``、RTL CSR map 和 testlist 中的 CSR stream 一致。
