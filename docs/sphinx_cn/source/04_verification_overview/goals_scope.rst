...
:status: draft
:source: docs/PROJECT_STATUS.md; docs/release-notes-v1.1.md; Makefile; dv/uvm/core_eh2/scripts/signoff.py; eh2_configs.yaml; dv/uvm/core_eh2/directed_tests/directed_testlist.yaml; dv/uvm/core_eh2/directed_tests/cosim_testlist.yaml; dv/uvm/core_eh2/riscv_dv_extension/testlist.yaml; dv/uvm/core_eh2/fcov/eh2_fcov_if.sv
...
本章只定义 EH2-Veri v1.1 的验证目标、已纳入 sign-off 的范围、当前配置覆盖边界和不覆盖范围。
所有数字来自当前仓库中的 release/status 文档、gate 脚本和 2026-05-19 01:02
VCS 主线 demo 实测摘要，不能用旧审计记录或未来目标替代。
尤其注意三点：

* 当前 release 版本是 `v1.1`，release 日期是 `2026-05-19 01:02`。
...
* 共享状态：release status 表、Makefile gate 变量、YAML testlist、coverage interface。

§2  v1.1 sign-off 目标
----------------------

v1.1 的目标不是“列出 EH2 所有可能功能”，而是在当前验证平台中形成可复演的 sign-off
...
* smoke、cosim、lint、csr_unit 和 syn 仍属于 9 stage sign-off；本节只列出用户最常
  复核的 release-facing 数字，逐 stage gate 由 :ref:`signoff_flow` 展开。

接口关系：

* 被调用：sign-off 报告、release note、验收门。
* 调用：无脚本调用；该表是汇总证据。
...
当前 Makefile 的 coverage gate 和 release coverage result 是两类不同数字：
gate 是 sign-off 判断门限，release result 是已达成的覆盖率。文档必须同时保留二者差异。
...
* 第 L148 行：注释说明旧 Makefile 曾写死 85/50，但 v1.1 release 不采用该门限。
...
* 被调用：coverage gate、release status、HTML report。
...
cosim proof list 由 7 个测试组成，release status 中 cosim 为 7/7。该列表使用
...
riscv-dv extension testlist 定义随机和混合 directed proof。v1.1 release 数字为
...
block-level LEC summary。当前 release status 记录 formal 46/46 和 syn 31635/31635。
...
以下项目不属于当前 EH2-Veri v1.1 sign-off 范围；这些边界来自源码缺口或当前 stage 列表，
不是对 EH2 core 能力的否定。
...
.. list-table::
   ...
   * - 全 profile sign-off
     - release status 是 v1.1 sign-off 数字
     - 不声明四个 `eh2_configs.yaml` profile 都完成 full sign-off。

接口关系：

* 被调用：scope review、risk review、release note。
