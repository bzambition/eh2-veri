...
.. tabs::
   ...
   .. tab:: 入门：VCS 默认路径
      ...
      这条路径是快速上手的默认答案。它生成 VCS ``simv``，跑 smoke，然后用
      quick profile 进入 sign-off gate；release 数字默认按 VCS/URG dashboard
      解释。
   ...
   .. tab:: 进阶：NC 备选路径
      ...
      这条路径用于 simulator cross-check、NC 波形调试和 IMC coverage 审计。
      它不改变“VCS/URG 是默认 release 参考”的事实；报告中应明确标注 NC branch
      metric 与 VCS BRANCH metric 不同构。

逐段解释：

* 两个 tab 都固定从 `/home/host/eh2-veri` 工作区开始。
* `source env.sh` 把 `EH2_VERIF_ROOT`、`RV_ROOT`、`GCC_PREFIX` 和
  RISC-V GCC `PATH` 写入当前 shell。
* VCS 入门 tab 中 `NO_COSIM=1` 只跳过编译期 `libcosim.so` 链接，适合没有 Spike
  安装目录的 smoke bring-up。
* `make smoke` 会编译 testbench、编译 `tests/asm/smoke.hex`，再通过
  `run_regress.py --test smoke` 启动 RTL simulation。
* `PROFILE=quick` 对应 `signoff.py` 中的 `smoke` 与 `directed` 两个 stage；
  `SIMULATOR=nc` 时走同一 stage 选择，但 coverage/report 口径按 NC/IMC 标注。
...
.. tabs::
   ...
   .. tab:: Makefile NC 入口
      ...
      README 不把 NC 写进最短三行，是为了让新读者先建立 VCS/URG 默认主线。
      NC 入口仍由顶层 Makefile 支持，适合在第一次 smoke 跑通后做 simulator
      cross-check。
