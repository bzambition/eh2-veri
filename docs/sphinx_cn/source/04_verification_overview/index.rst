...
* 当前 v1.1 到底验证什么，哪些 stage 和 coverage gate 属于 sign-off 范围。
...
* 被调用：顶层中文手册 toctree、读者导航、release review 入口。
* 调用：:ref:`goals_scope`、:ref:`quickstart`、:ref:`ibex_capability_matrix`。
* 共享状态：v1.1 release 数字、sign-off stage、coverage gate、Ibex 对标边界。

§2  release 数字的来源
----------------------

本部分所有 release-facing 数字必须来自当前 status/release notes，而不能从旧审计、
旧构建目录或人工推断中提取。当前 project status 明确给出 v1.1、日期、top-level
...
* 第 L1-L5 行：当前 release 版本是 v1.1，日期是 2026-05-19 01:02。
...
* 共享状态：v1.1、2026-05-19 01:02、top-level PASS。
...
.. list-table::
   ...
   * - 子章
     ...
     - 不负责展开的内容
   * - :ref:`goals_scope`
     - 当前 v1.1 sign-off 覆盖哪些 stage、哪些配置、哪些 coverage/formal/LEC 数字。
...
.. code-block:: bash
   ...
   所有数字来自当前仓库中的 release/status 文档或 gate 脚本，不能用旧审计记录或未来目标替代。
   尤其注意三点：
...
* 第 L19-L22 行：该章固定 v1.1、2026-05-19 01:02、`PROFILE` 和 coverage gate，
...
.. code-block:: bash
   ...
   sign-off profile 变量名是 `PROFILE`，不是旧文档里的 `SIGNOFF_PROFILE`。
...
* 第 L20-L22 行：该章明确当前推荐命令是 `make smoke`、`make regress`、
  `make signoff`，并纠正 `SIGNOFF_PROFILE` 旧变量名。
