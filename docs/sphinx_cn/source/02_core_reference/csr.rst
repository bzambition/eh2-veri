...
**v2-59 CSR register block 全文行段级精读**
（``dv/uvm/cs_registers_eh2/reg_model/eh2_csr_reg_block.sv``）：

.. literalinclude:: ../../../../dv/uvm/cs_registers_eh2/reg_model/eh2_csr_reg_block.sv
   :language: systemverilog
   :linenos:
   :caption: dv/uvm/cs_registers_eh2/reg_model/eh2_csr_reg_block.sv:全文

逐段精读：

* L1-L18：文件头说明该模型把每个 EH2 CSR 建成真实 ``uvm_reg``，替代旧的 placeholder
  ``csr_desc_t`` 思路。每个寄存器提供 build、reset、prediction 和 reset-value comparison 能力；
  DUT 访问通过 ``csr_dpi_pkg`` 后门函数完成。
* L20-L23：include/import 段。``csr_dpi_imports.svh`` 暴露 DPI 入口，``uvm_macros.svh`` 与
  ``uvm_pkg`` 提供 UVM factory/report/register 基类，``csr_dpi_pkg`` 提供 ``csr_dpi_read`` 和
  ``csr_dpi_write``。
* L25-L43：``eh2_csr_reg`` 类声明。该类继承 ``uvm_reg``，只有一个 32-bit ``value`` field，
  并保存 name、addr、reset、WARL mask、read-only flag 和描述字符串。
* L45-L62：``configure_csr`` 与 ``build``。前者填充 per-register metadata；后者创建 ``value``
  field，并根据 ``csr_read_only`` 配置为 ``RO`` 或 ``RW``。这使 CSR unit 能用统一 UVM API 管理
  标准、PMP、debug 和 custom CSR。
* L64-L84：metadata getter、DPI read/write 和本地 WARL 计算。``read_dut``/``write_dut`` 用地址
  调 DPI；``get_warl_value`` 只用 reg model mask 计算期望值，避免从 DUT 读回值反推 golden。
* L86：结束 ``eh2_csr_reg``。此时单个 CSR 的数据结构、访问路径和 WARL helper 已完整定义。
* L89-L129：``eh2_csr_reg_block`` 类头和标准 M-mode CSR 句柄。该段声明 machine information、
  trap setup/handling、counter 和 HPM counter/event 的 ``uvm_reg`` handle。
* L130-L150：PMP CSR 句柄。``pmpcfg0`` 到 ``pmpcfg3`` 以及 ``pmpaddr0`` 到 ``pmpaddr15`` 都有
  独立 handle，便于 sequence 遍历时覆盖配置寄存器和地址寄存器。
* L152-L165：debug 与 trigger CSR 句柄。``dcsr``、``dpc``、``dscratch``、``tselect``、
  ``tdata``、``tinfo`` 和 ``tcontrol`` 与普通 M-mode CSR 分开声明，方便 debug-only 或 trigger
  权限测试定位。
* L166-L197：EH2 custom CSR 句柄。该段覆盖 ``mscause``、``mrac``、``mfdc``、``mcgc``、PIC、
  ECC/timer、hart start、debug status、fault data 和 ``mzext`` 等 EH2 专属寄存器，是本模型区别于
  Ibex/标准 privileged CSR 表的核心。
* L198-L205：lookup 容器与 constructor。``reg_by_name``、``reg_by_addr`` 和 ``reg_names`` 分别
  支撑按名查找、按地址查找和有序遍历；constructor 关闭 UVM coverage，因为本文件是 register model，
  不是 functional coverage 收集点。
* L207-L212：``build`` 开始并创建 ``CSR_Map``。地址基为 0，步长 4，little-endian；后续
  ``default_map.add_reg`` 会把 12-bit CSR address 放进该 map。
* L213-L228：machine information 与 trap setup CSR。``mvendorid``、``marchid``、``mimpid``、
  ``mhartid`` 为只读；``mstatus``、``mie``、``mtvec`` 和 ``mcounteren`` 用 WARL mask 描述可写位。
  注释把 ``mstatus`` 和 ``mtvec`` 的 mask 对回 RTL 行号。
* L230-L237：trap handling CSR。``mscratch``、``mepc`` 和 ``mcause`` 可写，``mtval`` 与 ``mip``
  只读；``mepc`` mask 清低 2 位，符合对齐语义。
* L238-L257：counter、HPM counter 和 HPM event。``mcountinhibit`` 只允许实现位，cycle/instret/
  hpmcounter/event register 使用全 32-bit mask，支持 CSR unit 对计数器相关 CSR 做访问验证。
* L259-L281：PMP config/address 注册。``pmpcfg`` mask 为 ``32'h9F9F_9F9F``，表达每个 PMP cfg byte
  的合法字段；16 个 ``pmpaddr`` 使用全 32-bit mask。
* L283-L295：debug 与 trigger 注册。``dcsr``、``dpc``、``dscratch``、``tselect``、``tdata``、
  ``tinfo`` 和 ``tcontrol`` 填入地址、reset、mask 和 RO/RW 属性。
* L297-L327：EH2 custom CSR 注册。该段给 PIC、ECC、timer、debug access/fault data、hart start、
  NMI delegation 和 feature-extension 相关 CSR 设置地址、reset、WARL mask 和只读属性；这些值是
  CSR unit reset/WARL/access matrix 的主要 golden 数据源。
* L329-L330：``lock_model`` 结束 build，冻结 register block，防止后续 sequence 或 test 动态改变
  register layout。
* L332-L348：``add_reg_impl`` helper。函数创建 ``eh2_csr_reg``、配置 UVM parent 与 CSR metadata、
  调用 ``build``，然后写入 output handle、default map、按名/按地址关联数组和有序名称列表。
  所有 CSR 都经由这一条路径注册，避免句柄表、地址表和遍历列表分叉。
* L350-L362：``find_by_name``、``find_by_addr`` 和 ``get_count``。sequence library 用这些 API
  根据 test 目标查 CSR，并统计当前模型中注册了多少个 CSR。
* L364-L376：``dump`` 诊断输出。函数按 ``reg_names`` 遍历，打印地址、名字、reset、mask、RO/RW
  和描述，最后打印总数；这适合在 CSR unit bring-up 时核对模型是否覆盖预期 CSR。
* L378：结束 ``eh2_csr_reg_block``。

接口关系：

* 被调用：CSR unit env/test/sequence 在 build 或 run phase 中创建该 register block，并通过
  ``reg_names``、``find_by_name``、``find_by_addr`` 遍历 CSR。
* 调用：UVM register layer、``csr_dpi_read``、``csr_dpi_write``、``default_map.add_reg`` 和 UVM
  report macro。
* 共享状态：CSR 地址/reset/WARL mask/read-only 属性必须与 ``eh2_dec_csr.sv``、TLU CSR flop、
  Spike CSR fixup、CSR 文档表格和 CSR unit scoreboard 保持一致；否则 reset、WARL、access matrix
  或 hazard sequence 会出现 mismatch。
...
**v2-57 CSR unit Makefile 全文行段级精读**
（``dv/uvm/cs_registers_eh2/Makefile``）：

.. literalinclude:: ../../../../dv/uvm/cs_registers_eh2/Makefile
   :language: makefile
   :linenos:
   :caption: dv/uvm/cs_registers_eh2/Makefile:全文

逐段精读：

* L1-L8：文件头定义 CSR unit 子环境用途：独立编译运行 CSR UVM sub-environment，真实例化
  DUT，并通过 DPI 做 CSR read/write。它显式说明参考 Ibex ``dv/cs_registers/Makefile``，并支持
  ``SIMULATOR=vcs|nc``。
* L9-L17：顶层变量。``EH2_ROOT`` 从当前目录上溯定位验证仓库，``CSR_DIR`` 指向当前 CSR unit
  目录，``SIGNOFF_OUT`` 供 sign-off 汇总传入；``SIMULATOR`` 默认 ``nc``，seed、verbosity 和
  ``OUT`` 控制单次仿真输出。
* L18-L27：工具与 RTL 根路径。VCS 默认路径导出 ``UVM_HOME``，NC 使用与顶层 Makefile 一致的
  Cadence UVM home；``EH2_RTL_ROOT`` 固定指向上游 Cores-VeeR-EH2 checkout，是迁移机器时最容易
  需要覆盖的变量。
* L28-L35：公共 include path。CSR env、reg model、reg driver、TB、tests 以及 EH2 snapshot
  default 目录都进入 ``+incdir``，保证 package、wrapper 和上游参数头在两种 simulator 下可见。
* L37-L43：VCS compile flags。该段打开 SystemVerilog、64-bit、timescale 和 UVM 1.2，再拼接
  ``INC_DIRS``。VCS 分支后续只补 source/top/output/log，不重复写 include 列表。
* L45-L65：源码清单。``EH2_RTL_SRC`` 只取 CSR DUT wrapper 必要的参数头、``eh2_def.sv`` 和
  ``eh2_dec_csr.sv``；``CSR_TB_SRC`` 再追加 DPI package、env cfg、scoreboard、reg block、
  sequence library、env、wrapper、TB top 和 4 个 test class。
* L67-L78：输出目录、phony target 和 help。``$(OUT)`` target 用 ``mkdir -p`` 创建 build 目录；
  ``help`` 只打印 compile、run-csr-unit、compliance 和 clean 入口，适合作为本地 quick reference。
* L80-L107：NC/Incisive 分支。``compile`` 使用 ``irun -64bit -uvmhome``、SV/assert/vh extension、
  include/source/top、``-elaborate``、``INCA_libs`` 和 compile log；``sim`` 重新调用 ``irun``，
  传入 UVM test、verbosity、seed 和 per-test log。命令前的 ``-irun`` 说明仿真失败不会立刻让
  make 停止，后续需要读取 log 或外层 compliance 汇总判断结果。
* L109-L131：VCS 分支。``compile`` 使用 ``vcs $(VCS_FLAGS)`` 生成 ``$(OUT)/simv``；``sim`` 运行
  该 simv，并传入相同的 UVM test、verbosity、seed 和 log 路径。它与顶层 UVM flow 共享 VCS/UVM
  口径，但只编译 CSR unit 所需的最小 RTL/TB 集合。
* L133-L140：单测和 all target。``run-csr-unit`` 固定 ``TEST=cs_registers_test``；
  ``run-csr-all`` 依次跑基础、access matrix、illegal 和 hazard 四个测试，每个测试使用不同 seed。
* L142-L148：sign-off compliance target。该 target 明确 4 tests × 5 seeds = 20 simulations，
  并调用 ``$(CSR_DIR)/scripts/run_compliance.py``；``SIGNOFF_OUT`` 透传给 Python runner，用于把结果
  汇入外层 sign-off 目录。
* L150-L153：clean target 删除 CSR unit 构建物、VCS/NC 临时目录、log、波形、coverage report 和
  ``report.json``，最后打印清理完成。它只清 CSR unit 相关 artifact，不负责仓库级 EDA 清理。

接口关系：

* 被调用：工程师在 ``dv/uvm/cs_registers_eh2`` 下运行 ``make compile``、``make run-csr-unit``、
  ``make run-csr-all`` 或 ``make compliance``。
* 调用：VCS ``vcs``、Cadence ``irun``、Python ``run_compliance.py``、shell ``mkdir`` 和 ``rm``。
* 共享状态：依赖 ``EH2_RTL_ROOT`` 的上游 RTL snapshot、CSR unit 源文件、``OUT`` 目录、UVM test
  名、seed 和 ``SIGNOFF_OUT``；sign-off 结论以 compliance Python 汇总为准，不以单条 echoed exit
  文本替代。
