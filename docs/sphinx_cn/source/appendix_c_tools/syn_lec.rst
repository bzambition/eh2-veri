...
§14  v2-16 LEC 诊断脚本逐段补齐
--------------------------------------------------------------------------------

本节补齐 v2-15 审计中缺少逐段解释的 LEC 诊断脚本。它们多数不是当前 release
读取的唯一事实源；当前 release gate 仍以 block-level ``lec_summary.txt`` 和
``31635/31635 PASS`` 为准。本节的价值在于解释这些历史/诊断 Tcl 如何定位
packed-port、EXU matching 和 timeout 问题。

§14.1  EXU block-level 诊断脚本
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_analyze_timeout.tcl
   :language: text
   :lines: 1-34
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_analyze_timeout.tcl:L1-L34

逐段精读：

* L1-L8：脚本 source ``lec_common.tcl``，把 verification timeout 设为 3 分钟并降低 effort。
  它用于快速分析 EXU timeout，不用于最终证明。
* L10-L15：读取 ``eh2_exu`` reference/implementation 和 SVF，沿用 block-level LEC
  的公共加载路径。
* L17-L27：手工匹配 ``exu_npc_e4``、``exu_flush_path_final`` 和 early flush 位宽差异。
  这些 user match 解释了 EXU packed array flatten 后的关键 bit 对齐策略。
* L29-L34：运行 ``match``、``verify -level 1``，并输出 status、unverified、analyze_points
  和 analysis_results 四类诊断报告。

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_level0_diag.tcl
   :language: text
   :lines: 1-30
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_level0_diag.tcl:L1-L30

逐段精读：

* L1-L8：同样加载 block-level 公共环境，但不设置 timeout/effort，目标是 level-0 快速诊断。
* L14-L25：user match 逻辑与 timeout 分析脚本一致，确保两个诊断入口观察同一批 EXU
  compare point。
* L27-L30：``verify -level 0`` 只做浅层验证并输出 failing/unverified point，适合先确认
  失败是否来自 matching，而不是完整证明深度。

§14.2  top-level matching 诊断脚本
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_diag3.tcl
   :language: text
   :lines: 1-43
   :linenos:
   :caption: syn/scripts/lec_diag3.tcl:L1-L43

逐段精读：

* L1-L18：读取 wrapper/netlist 前设置 search path、SV 标准和 relaxed mode。这是早期
  top-level LEC 探索脚本的共同骨架。
* L20-L25：分别读取 reference ``eh2_dc_wrapper.sv`` 和 implementation ``eh2_synth.v``，
  并把 top 设置为 ``eh2_veer``。
* L29-L41：打印匹配 ``*btb*`` 和 ``*trace_rv_i*`` 的端口名，用于确认 Formality 展开后
  真实端口名称。它是诊断脚本，不是 sign-off gate。

.. literalinclude:: ../../../../syn/scripts/lec_matching_v2.tcl
   :language: text
   :lines: 1-45
   :linenos:
   :caption: syn/scripts/lec_matching_v2.tcl:L1-L45

逐段精读：

* L1-L25：读取 top-level wrapper/netlist，与 ``lec_diag3.tcl`` 相同。
* L27-L37：注释说明目标是处理 2D packed array port 展平后名字不一致的问题。
* L40-L45：``set_user_match_type -type ports`` 尝试按 port 类型/宽度匹配，而不是逐个名字匹配。
  这个实验帮助定位 top-level packed port 限制，但最终 release 改走 block-level。

.. literalinclude:: ../../../../syn/scripts/lec_matching_v3.tcl
   :language: text
   :lines: 1-41
   :linenos:
   :caption: syn/scripts/lec_matching_v3.tcl:L1-L41

逐段精读：

* L1-L25：仍是 top-level reference/implementation 加载骨架。
* L27-L32：先执行 ``automatch``，再执行 ``match``，观察自动匹配能否解决端口展开问题。
* L34-L41：打印 unmatched count 并输出 status/failing points。若这里仍有大量 unmatched，
  说明需要 block-level 或手工 user match，而不是调整 coverage/sign-off 口径。

§14.3  RC4 末期 top-level 实验脚本
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_rc4_final.tcl
   :language: text
   :lines: 1-55
   :linenos:
   :caption: syn/scripts/lec_rc4_final.tcl:L1-L55

逐段精读：

* L1-L7：注释记录 RC3 到 RC4 的历史数字，用于解释为什么要诊断剩余 194 failing points。
* L11-L17：把 Formality 临时目录重定向到 ``lec_rc4_final_run``，避免工作文件污染主 build。
* L19-L36：读取 technology library、reference wrapper 和 implementation netlist。
* L38-L53：执行 match、``analyze_points``、verify，并输出 final report、failing detail
  和 unverified detail。该脚本是 top-level 收敛历史，不替代当前 block-level release gate。

.. literalinclude:: ../../../../syn/scripts/lec_rc4_v2.tcl
   :language: text
   :lines: 1-41
   :linenos:
   :caption: syn/scripts/lec_rc4_v2.tcl:L1-L41

逐段精读：

* L1-L14：脚本说明这是 keep-2D netlist 实验，尝试保留二维端口结构来降低 matching 难度。
* L16-L28：读取 ``eh2_dc_wrapper.sv`` 和 ``eh2_synth_keep2d.v``，implementation 文件名已经
  暗示该实验区别于普通 synth netlist。
* L30-L41：match/verify 后把报告写成 ``r3c_lec_keep2d_*``。如果报告失败，应归类为
  top-level 实验失败，而不是 block-level LEC gate 失败。

§14.4  ``test_synth.tcl`` — 最小综合读入探针
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/test_synth.tcl
   :language: tcl
   :lines: 1-1
   :linenos:
   :caption: syn/test_synth.tcl:L1-L1
...
§16  v2-50 block-level LEC 小脚本族全文行段级精读
--------------------------------------------------------------------------------

本节补齐 8 个 block-level Formality 小入口的全文源码。它们不是独立的 LEC 框架，而是
``lec_common.tcl`` 之上的 per-block wrapper：每个脚本选择 reference top，读入对应
implementation netlist，加载同名 SVF，执行 ``match``/``verify``，最后把报告写回
``syn/build/lec_blocklevel/reports``。因此读这些脚本时要重点看两类差异：一类是 top/label
选择，另一类是为解决 packed array 或 datapath 复杂度而加入的 user match 和验证策略。

§16.1  ``lec_dbg.tcl`` — DBG block LEC 最小闭环全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_dbg.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_dbg.tcl:全文

逐段精读：

* L1-L3：关闭 ``sh_continue_on_error`` 并加载 ``lec_common.tcl``。前者保证 Formality 命令失败时
  不继续产出误导性报告；后者提供 ``r3c_read_impl``、``r3c_set_impl_top``、``r3c_load_svf`` 和
  ``r3c_write_reports``。
* L4-L9：把本次 block 标记为 ``dbg``，reference top 设为 ``r:/WORK/eh2_dbg``，再读入
  ``eh2_dbg`` 的 implementation netlist、设置 implementation top，并加载同名 SVF。
* L11-L19：依次执行 ``match``、``verify``、``r3c_write_reports dbg`` 和 ``exit 0``。该脚本没有
  user match 或特殊 timeout，说明 DBG 在当前 release 中走标准 block-level LEC 骨架即可闭环。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 的 Formality loop 通过 ``fm_shell -f`` 调用。
* 调用：``lec_common.tcl`` 中的 R3-C helper，以及 Formality 原生命令 ``set_top``、``match``、
  ``verify``。
* 共享状态：读取 ``eh2_dbg`` 的 synthesized netlist 与 SVF；写 ``dbg`` 前缀的 report 文件。

§16.2  ``lec_dma.tcl`` — DMA controller block LEC 最小闭环全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_dma.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_dma.tcl:全文

逐段精读：

* L1-L3：启动方式与 ``lec_dbg.tcl`` 相同，先禁止错误后继续，再加载 block-level 公共环境。
* L4-L9：本脚本的 top 是 ``eh2_dma_ctrl``，report label 是 ``dma``。``set_top`` 先锁定
  reference design，随后 helper 读取 implementation、设置 implementation top，并加载
  ``eh2_dma_ctrl.svf``。
* L11-L19：脚本直接 ``match``、``verify`` 并调用 ``r3c_write_reports dma``。DMA controller
  不在这里设置额外 datapath effort；若失败，应先看公共读入、SVF 和报告，而不是假设需要手工匹配。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 中 label 为 ``dma`` 的 Formality 子任务。
* 调用：``lec_common.tcl`` helper 与 Formality ``match``/``verify``。
* 共享状态：读取 ``eh2_dma_ctrl`` netlist/SVF；写 ``dma`` 前缀报告。

§16.3  ``lec_pic.tcl`` — PIC controller block LEC 最小闭环全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_pic.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_pic.tcl:全文

逐段精读：

* L1-L3：与 DBG/DMA 一样，脚本只做 per-block policy，公共路径由 ``lec_common.tcl`` 承担。
* L4-L9：PIC 的 reference top 是 ``eh2_pic_ctrl``。这个名字必须与 DC block synthesis 阶段输出的
  netlist、DDC 和 SVF basename 对齐，否则 ``r3c_read_impl`` 或 ``r3c_load_svf`` 会在公共层失败。
* L11-L19：执行标准 ``match``/``verify``/``r3c_write_reports pic`` 闭环。PIC 中断寄存器和优先级逻辑
  的证明深度不在脚本中硬编码，仍由 Formality 默认策略和 SVF 映射共同决定。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 中 label 为 ``pic`` 的 Formality 子任务。
* 调用：``lec_common.tcl`` helper 与 Formality ``match``/``verify``。
* 共享状态：读取 ``eh2_pic_ctrl`` netlist/SVF；写 ``pic`` 前缀报告。

§16.4  ``lec_exu_div.tcl`` — EXU divider 子块 LEC 全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_div.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_div.tcl:全文

逐段精读：

* L1-L4：先设置 ``R3C_PRELOAD_SVF_TOP=eh2_exu_div_ctl``，再加载公共脚本。这个环境变量让公共层在
  读入 reference/implementation 前后能预加载 divider 对应 SVF，避免综合变换信息丢失。
* L5-L13：选择 top ``eh2_exu_div_ctl``、report label ``exu_div``，并把验证 timeout 设为
  ``0:5:0``。divider 是 datapath 子块，脚本显式给出较长但有限的证明窗口。
* L15-L23：执行 ``match``、``verify -level 1``、写 report 并退出。``-level 1`` 比默认浅层诊断更强，
  但仍保留为 block 子问题，避免回到 monolithic EXU 的匹配复杂度。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 中 label 为 ``exu_div`` 的 Formality 子任务。
* 调用：``lec_common.tcl`` helper、SVF 预加载路径和 Formality ``verify -level 1``。
* 共享状态：读取 ``eh2_exu_div_ctl`` netlist/SVF；写 ``exu_div`` 前缀报告。

§16.5  ``lec_exu_mul.tcl`` — EXU multiplier 子块 LEC 全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_mul.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_mul.tcl:全文

逐段精读：

* L1-L7：设置 MUL 子块 SVF 预加载 top，加载公共层，并把 top/label 设为
  ``eh2_exu_mul_ctl``/``exu_mul``。
* L8-L20：允许调用者通过 ``R3C_REPORT_LABEL``、``R3C_FM_STRATEGY`` 和
  ``R3C_FM_PASSING_MODE`` 覆盖 report label、Formality alternate strategy 与 passing mode。
  这三个开关是 MUL 收敛时的实验入口，默认不改变主线策略。
* L22-L35：读入 implementation、加载 SVF、``match`` 后执行 ``verify -level 1``，再写出 label
  对应 report。与 divider 一样，MUL 被拆成子块证明，以降低单体 EXU datapath 的复杂度。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 中 label 为 ``exu_mul`` 的 Formality 子任务，也可由人工调试时
  带环境变量单独调用。
* 调用：``lec_common.tcl`` helper、Formality strategy/passing-mode app var 与 ``verify -level 1``。
* 共享状态：读取 ``eh2_exu_mul_ctl`` netlist/SVF；写 ``exu_mul`` 或环境变量指定 label 的 report。

§16.6  ``lec_exu_alu.tcl`` — EXU ALU 子块 packed-struct 匹配全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_alu.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_alu.tcl:全文

逐段精读：

* L1-L12：设置 ``eh2_exu_alu_ctl`` 的 SVF 预加载 top，加载公共环境，读入 implementation top 并加载
  SVF。ALU 子块在进入 match 前已经完成 reference/implementation 双方的 top 对齐。
* L14-L31：定义 reference/implementation top path，随后手工匹配 ``predict_p_ff`` packed-struct
  字段。reference 侧字段名为 ``ataken``、``misp``、``hist[0]``、``hist[1]``，implementation 侧被
  flatten 成 bit index 8、5、11、12；脚本把 user match 数写到
  ``lec_exu_alu_user_match_count.txt`` 供审计。
* L33-L41：执行 ``match``、默认 ``verify``、写 ``exu_alu`` report 并退出。这里的关键不是验证级别，
  而是确保 packed-struct 展平后的 4 个 compare point 被显式对齐。

接口关系：

* 被调用：``syn/Makefile:block_lec`` 中 label 为 ``exu_alu`` 的 Formality 子任务。
* 调用：``set_user_match`` 对齐 packed-struct 字段，并调用 ``lec_common.tcl`` report helper。
* 共享状态：读取 ``eh2_exu_alu_ctl`` netlist/SVF；写 ``exu_alu`` report 和 user-match count 文件。

§16.7  ``lec_exu.tcl`` — monolithic EXU block LEC 与 packed-array 匹配全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu.tcl:全文

逐段精读：

* L1-L9：加载公共层后选择 monolithic top ``eh2_exu``，读入 implementation、设置 implementation
  top 并加载同名 SVF。这个脚本保留单体 EXU 尝试，但 release summary 更偏向 ALU/MUL/DIV 子块结果。
* L11-L28：建立 reference/implementation path，并对 ``exu_npc_e4`` 与
  ``exu_flush_path_final`` 的 1..31 位逐项 ``set_user_match``；另把
  ``exu_flush_path_final_early[0][31]`` 匹配到 implementation 的 bit 0。user match 数写入
  ``lec_exu_user_match_count.txt``，便于解释 packed-array flatten 的人工对齐范围。
* L30-L38：执行 ``match``、``verify -level 1`` 和 ``r3c_write_reports exu``。如果该脚本失败，
  应先看 user match 与 unmatched report，而不是直接否定拆分后的 EXU 子块 gate。

接口关系：

* 被调用：历史或诊断性的 block-level EXU Formality 子任务；当前 summary 脚本会优先使用
  ``exu_alu``、``exu_mul``、``exu_div`` 子块结果替代单体 EXU。
* 调用：``set_user_match``、``lec_common.tcl`` helper 和 Formality ``verify -level 1``。
* 共享状态：读取 ``eh2_exu`` netlist/SVF；写 ``exu`` report 与 user-match count 文件。

§16.8  ``lec_exu_match_diag.tcl`` — EXU matching-only 诊断全文
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../syn/scripts/lec_blocklevel/lec_exu_match_diag.tcl
   :language: text
   :linenos:
   :caption: syn/scripts/lec_blocklevel/lec_exu_match_diag.tcl:全文

逐段精读：

* L1-L9：以 ``eh2_exu`` 为 top 读入 reference/implementation 并加载 SVF，前置环境与
  ``lec_exu.tcl`` 相同。
* L11-L20：复用 monolithic EXU 的 ``exu_npc_e4`` 与 ``exu_flush_path_final`` 位级 user match，
  但不加入 early flush 单点匹配。该差异让它更适合观察基础 packed-array matching 状态。
* L22-L27：执行 ``match`` 后只输出 matched、unmatched 和 status 报告，不执行 ``verify``。
  这说明它是 matching 诊断脚本，不能作为 sign-off pass/fail gate。

接口关系：

* 被调用：人工定位 EXU unmatched point 时单独运行，不属于最终 sign-off 证明集合。
* 调用：``set_user_match``、``match``、``report_matched_points``、``report_unmatched_points`` 和
  ``report_status``。
