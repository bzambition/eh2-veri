...
* 第 5-6 行：文档 Python 包需求只有 `sphinx>=7.0` 和 `rinohtype>=0.5.5`。
  当前运行环境若使用 Sphinx 4.5.0，也只能说明本地构建工具链状态，不改变 requirements
  文件的内容。
...
.. list-table::
   ...
   * - `make manual`
     ...
     - `docs/requirements-docs.txt:L1-L6`、`Makefile:L495-L507`
   * - full sign-off
     - 上述工具按 stage 组合
