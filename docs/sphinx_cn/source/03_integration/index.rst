...
平台负责人。当前叙述以 VCS/URG 作为默认 release 参考；NC/Incisive 是完整备选
simulator，可用于 smoke、regress、sign-off、demo、coverage cross-check 和波形调试。
...
.. list-table:: 集成章节约束矩阵
   ...
   * - 约束
     ...
     - 文档处理方式
   * - 工具主线
     - 顶层 ``SIMULATOR ?= vcs``，``demo/signoff`` 默认使用 VCS 主线
     - quick start 优先展示 VCS；NC 作为备选 tab 展示差异
   * - 配置入口
...
v2-10 起，读者可以先按默认 VCS tab 跑通，再切到 NC tab 对比备选 simulator 的
日志、coverage database 和波形产物。

.. tabs::

   .. tab:: VCS 默认 release 路径
      ...
      .. code-block:: bash
         ...
         # 最小 smoke；SIMULATOR 默认就是 vcs
         make smoke

         # directed 回归；开发阶段可关覆盖率加速
         make regress TESTLIST=directed PARALLEL=4 COV=0

         # 快速 sign-off profile，release 参考仍按 VCS/URG 解释
         make signoff PROFILE=quick PARALLEL=4

         # 完整 demo，生成 HTML 报告
         make demo

      VCS 路径的覆盖率数据库是 ``.vdb``，合并工具是 URG，release 表格中的
      LINE/BRANCH/TOGGLE/ASSERT/FSM/GROUP/OVERALL 数字默认来自这一条路径。

   .. tab:: NC/Incisive 备选与波形路径
      ...
      .. code-block:: bash
         ...
         # NC smoke；WAVES=1 时加载 nc_waves.tcl
         make smoke SIMULATOR=nc WAVES=1

         # NC directed 回归，用于 simulator cross-check
         make regress SIMULATOR=nc TESTLIST=directed PARALLEL=4 COV=0

         # NC 备选签核；报告必须标注 IMC 与 branch/block 口径差异
         make signoff SIMULATOR=nc PROFILE=quick PARALLEL=4

         # NC demo，用于备选 dashboard 与波形调试
         make demo SIMULATOR=nc

      NC/Incisive 路径的 coverage database 是 ``cov_work``，合并工具是 IMC。
      ``cov_full_nc.ccf`` 会限定 DUT scope，但 NC 152 IMC 没有独立 BRANCH metric，
      因此不能把 NC dashboard 中的 block/branch 行为写成 VCS BRANCH 数字。
...
* NC/Incisive 是完整备选 simulator，但 release 参考仍默认采用 VCS/URG。若未来把
  NC/IMC 数字提升为主 release 口径，需要单独 ADR、工具真实性验证和 sign-off gate
  更新。
