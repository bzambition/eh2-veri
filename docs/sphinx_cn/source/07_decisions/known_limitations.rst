...
NC 不参与 sign-off coverage 的约束在 ``merge_cov.py`` 中写成显式边界：

.. literalinclude:: ../../../../dv/uvm/core_eh2/scripts/merge_cov.py
   :language: python
   :lines: 86-111
   :caption: merge_cov.py:86-111 - 非 VCS coverage merge 处理
...
.. code-block:: bash
   ...
   # NC 只用于单测波形，不更新 release coverage 结论
   make regress TESTLIST=directed SIMULATOR=nc WAVES=1 ITERATIONS=1
...
.. list-table:: 已知限制对照
   ...
   * - simulator fallback
     ...
     - 当前 release 只有 VCS/URG 是 coverage/sign-off 主线，NC 只看波形
   * - LEC
     - top-level 路径更直接
...
.. list-table:: 当前已知限制
   ...
   * - LIM-07
     - controlled
     - NC/Incisive 不参与 release coverage
     - Makefile 和 merge_cov.py 明确 VCS-only sign-off coverage
     - 保留 NC 波形调试文档，不扩展为签核路径
   * - LIM-08
     - future
