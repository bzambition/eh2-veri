...
.. tabs::
   ...
   .. tab:: VCS/URG release 参考
      ...
      这条路径是 2026-05-19 demo 数字的来源。LINE、BRANCH、TOGGLE、ASSERT、
      FSM、GROUP 和 OVERALL 全部来自 DUT subtree 的 URG dashboard。

   .. tab:: NC/IMC 备选 cross-check

      .. code-block:: text

         make signoff SIMULATOR=nc COV=1
            |
            +-- make compile SIMULATOR=nc COV=1
            |     |
            |     +-- irun/xrun coverage switch
            |     +-- -covfile dv/uvm/core_eh2/cov_full_nc.ccf
            |     `-- cov_work directory
            |
            +-- signoff.py --coverage
            |     |
            |     +-- collect cov_work/*.ucd
            |     +-- imc report -metrics ... -cumulative on
            |     `-- dashboard.txt with NC metric caveats

      这条路径用于 simulator cross-check 和备选签核。NC 152 IMC 的 branch 与
      block 口径不等同于 VCS BRANCH，因此 release review 必须保留口径标注。
