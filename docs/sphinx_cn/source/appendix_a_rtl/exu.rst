...
§11  v2-19 ``eh2_exu.sv`` 全文段落级精读
--------------------------------------------------------------------------------

``eh2_exu.sv`` 是 EXU 顶层 glue，负责 I0/I1 primary ALU、secondary ALU、MUL/DIV、
branch prediction repair、flush path 和 PMU branch event 的组合。此前章节按子模块
解释 ALU/MUL/DIV，但 v2-19 将顶层全文纳入，确保 bypass、flush 优先级和 predictor
feedback 没有被跳过。

.. literalinclude:: ../../../../../Cores-VeeR-EH2/design/exu/eh2_exu.sv
   :language: systemverilog
   :linenos:
   :caption: /home/host/Cores-VeeR-EH2/design/exu/eh2_exu.sv:全文

逐段精读：

* L1-L16：版权、include 和 package import。EXU 顶层读取 ``eh2_def.sv`` 中的
  ALU/MUL/DIV/predict packet 定义。
* L17-L59：模块头、clock/reset、clock enable、secondary/branch clock gating 和
  debug RS1 mux 端口。DEC 把 clock gate pulse 和 debug write data select 送到这里。
* L60-L154：LSU result、predict packet、bypass、secondary ALU、MUL/DIV、operand、
  LSU/CSR select、ALU packet 和 flush/commit 输入。该段是 DEC 到 EXU 的完整数据面。
* L155-L234：EXU 输出端口。结果回传 DEC/LSU/CSR，flush final/early 返回 IFU/DEC，
  MUL/DIV 写回、branch predictor repair packet 和 PMU branch event 从这里离开。
* L238-L320：内部 operand、flush、prediction pipeline、valid、GHR 和 final predictor
  metadata 声明。``PREDPIPESIZE`` 把 FGHR、index、btag、toffset 打包进可流水化字段。
* L325-L379：D-stage operand mux。I0/I1 RS1/RS2 根据 decode bypass、debug write data、
  CSR read clear、LSU operand select、MUL/DIV select 和 immediate 形成 ALU/MUL/DIV 输入。
* L380-L408：clock enable 拆分、ALU packet pipeline 和 branch prediction pipeline
  的寄存器准备。I0/I1 data/control enable 分别驱动 E1-E4 阶段。
* L409-L465：``always_comb`` 预测修正逻辑。该段根据 predict packet、ALU branch 结果、
  upper/lower flush 和 PC/target 计算 mispredict、taken、flush path 和 correct NPC。
* L466-L545：predict pipe assignment、PMU 输出、ALU/MUL/DIV 子模块和 pipeline register
  连接。这里把 ``i0_predict_p_*``、``i1_predict_p_*`` 推进到 E4，并生成 branch PMU 事件。
* L546-L607：E2/E3 bypass final mux、taken 判断、GHR update 和 branch metadata 准备。
  load result、bypass data 和 ALU stage data 在这里决定后续 branch/flush 是否正确。
* L608-L639：E4 branch feedback 输出。I0/I1 的 hist、bank、error、valid、mispredict、
  call/ret、way 和 FGHR 被拆给 DEC/IFU，用于 BTB/BHT 修复和覆盖率采样。
* L640-L692：per-thread final prediction/flush generate。每个 thread 选择 final
  mispredict packet、after-flush GHR、valid 条件和 GHR next-state，支撑双线程 flush
  优先级。
* L693-L725：final/early flush path 选择。正式 flush 优先考虑 WB lower flush，再考虑
  E2 upper flush；early flush 可在配置打开时提前暴露 E1/E4 flush，降低前端重定向延迟。
* L726-L792：ALU/MUL/DIV 实例和 pipeline 收尾。该段把 primary/secondary ALU、multiply、
