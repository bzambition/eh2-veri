...
* `rtl_simulation.yaml` 是 simulator mapping，包含 `vcs`、`xlm`、`nc`、`questa`
  四个 simulator key，每个 key 下有 `compile` 和 `sim` 阶段。
...
§9 v2-9 YAML/coverage 配置审计
--------------------------------------------------------------------------------

v2-9 对 YAML 与 coverage 配置做一次缺口补齐，重点是 ``rtl_simulation.yaml`` 的
``nc`` 段、NC coverage CCF、VCS FSM 配置和 functional coverage waiver。此前章节已经
逐段讲了 VCS/Xcelium/Questa 与 directed/cosim testlist；本节把尚未显式列入的配置文件补齐为
review checklist。
...
.. list-table::
   ...
   * - 文件
     ...
     - 审计结论
   * - ``dv/uvm/core_eh2/yaml/rtl_simulation.yaml``
     - ``run_rtl.py`` / ``compile_tb.py``
     - 4 个 simulator key：``vcs``、``xlm``、``nc``、``questa``。
   * - ``dv/uvm/core_eh2/cov_full_nc.ccf``
     - ``rtl_simulation.yaml:nc.cov_opts``
     - NC/Incisive coverage cross-check 配置，不改变 VCS/URG 默认 release 参考。
   * - ``dv/uvm/core_eh2/cov_fsm.cfg``
     - VCS ``-cm_fsmcfg``
     - FSM extraction 表，服务 VCS structural FSM coverage。
   * - ``dv/uvm/core_eh2/cov_fsm_reset_filter.cfg``
     - VCS ``-cm_fsmresetfilter``
     - 过滤 active-low reset transition。
   * - ``dv/uvm/core_eh2/fcov/cov_waivers/*.yaml``
     - coverage triage / release review
     - 当前 2 条 active waiver，均有 architectural reason。

关键代码（``dv/uvm/core_eh2/yaml/rtl_simulation.yaml:L114-L137``）：

.. literalinclude:: ../../../../dv/uvm/core_eh2/yaml/rtl_simulation.yaml
   :language: yaml
   :lines: 114-137
   :caption: /home/host/eh2-veri/dv/uvm/core_eh2/yaml/rtl_simulation.yaml:L114-L137

逐段解释：

* 第 L114-L117 行：``nc`` key 是 Cadence Incisive/irun 入口。源码注释仍写有
  DEBUG-ONLY 历史字样，但当前命令模板已经包含 compile/sim/cov opts，可用于完整备选路径。
* 第 L128-L137 行：compile command 使用 ``irun -64bit``、Cadence UVM 1.2、RTL/TB
  filelist、``INCA_libs``、cosim include 和 ``libcosim.so``。

关键代码（``dv/uvm/core_eh2/yaml/rtl_simulation.yaml:L151-L169``）：

.. literalinclude:: ../../../../dv/uvm/core_eh2/yaml/rtl_simulation.yaml
   :language: yaml
   :lines: 151-169
   :caption: /home/host/eh2-veri/dv/uvm/core_eh2/yaml/rtl_simulation.yaml:L151-L169

逐段解释：

* 第 L151-L164 行：NC sim command 单步运行 full irun pipeline，并传入
  ``+UVM_TESTNAME``、``+bin``、``+seed``、``+timeout_ns`` 和 UVM verbosity。
* 第 L165-L168 行：NC sim coverage 打开 ``+enable_eh2_fcov=1``，使用
  ``-coverage all``、``-covworkdir``、``-covtest`` 和 ``cov_full_nc.ccf``。
* 第 L169 行：波形脚本是 :file:`dv/uvm/core_eh2/nc_waves.tcl`。

关键代码（``dv/uvm/core_eh2/fcov/cov_waivers/dual_issue_presync_stall_cross_waiver.yaml:L7-L18``）：

.. literalinclude:: ../../../../dv/uvm/core_eh2/fcov/cov_waivers/dual_issue_presync_stall_cross_waiver.yaml
   :language: yaml
   :lines: 7-18
   :caption: /home/host/eh2-veri/dv/uvm/core_eh2/fcov/cov_waivers/dual_issue_presync_stall_cross_waiver.yaml:L7-L18

逐段解释：

* 第 L7-L10 行：waiver 名称和 coverage point 指向 ``uarch_cg.stall_cross``。
