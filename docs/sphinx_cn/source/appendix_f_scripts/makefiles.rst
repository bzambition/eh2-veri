...
§17  v2-35 UVM scripts Makefile 片段全文行段级精读
--------------------------------------------------------------------------------

本节补齐 ``dv/uvm/core_eh2/scripts`` 下 staged wrapper include 的 Makefile 片段。
这些片段由 ``dv/uvm/core_eh2/wrapper.mk`` include，不直接作为顶层 makefile 调用；
它们的职责是把 metadata、riscv-dv 生成、TB 编译、RTL 运行、log 检查、coverage 合并和
结果汇总拆成可增量重跑的 stamp target。

§17.1  ``get_meta.mk`` — metadata 字段访问器
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/scripts/get_meta.mk
   :language: make
   :linenos:
   :caption: dv/uvm/core_eh2/scripts/get_meta.mk:全文

逐段精读：

* L1-L3：文件头说明该片段提供 Ibex-compatible metadata accessor，目标是服务 EH2
  ``wrapper.mk``。
* L4-L9：``get-metadata-variable`` macro 通过 ``python3 ./scripts/metadata.py`` 执行
  ``print_field`` 操作。调用方传入字段名，脚本从 ``$(METADATA-DIR)`` 读取 metadata。
* L10-L12：``get-meta`` 用 ``$(shell ...)`` 包装上一层 macro，使 Make 变量赋值处可以直接
  写 ``$(call get-meta,field)``。这会在 Make expansion 阶段读取 metadata，而不是等 recipe
  执行时才读取。

§17.2  ``util.mk`` — wrapper 共用 make helper
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/scripts/util.mk
   :language: make
   :linenos:
   :caption: dv/uvm/core_eh2/scripts/util.mk:全文

逐段精读：

* L1-L2：文件头说明这是 ``wrapper.mk`` 共享的小型工具片段。
* L4-L8：根据 ``VERBOSE`` 设置 ``verb``。``VERBOSE=1`` 时 recipe 命令不加 ``@``，
  便于调试；默认情况下 ``verb=@``，只显示阶段性 ``echo``。
* L10-L15：``dump-vars`` 是调试 target，打印 ``OUT-DIR``、``METADATA-DIR``、
  ``SIMULATOR`` 和 ``TEST``。它不生成文件，也不影响 staged dependency graph。

§17.3  ``riscvdv.mk`` — core config 与随机指令生成阶段
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/scripts/riscvdv.mk
   :language: make
   :linenos:
   :caption: dv/uvm/core_eh2/scripts/riscvdv.mk:全文

逐段精读：

* L1-L8：文件头说明该片段提供 riscv-dv 随机指令生成器的 build/run 目标，并由
  ``wrapper.mk`` include。
* L9-L23：定义三个阶段入口：``core_config`` 依赖 ``core.config.stamp``，
  ``instr_gen_build`` 依赖 ``instr.gen.build.stamp``，``instr_gen_run`` 依赖所有
  ``riscvdv-test-asms``。空的 ``riscvdv-test-asms``/``riscvdv-test-bins`` 由
  ``wrapper.mk`` 根据 metadata 追加。
* L24-L35：instruction generator build 的变量依赖写入 ``.instr_gen.vars.mk``。
  ``vars-prereq`` 会根据 ``SIMULATOR`` 和 ``SIGNATURE_ADDR`` 生成变量变化前置条件，
  避免参数变化后误用旧 generator build。
* L36-L45：``instr.gen.build.stamp`` 依赖 riscv-dv 文件、core config stamp 和
  ``build_instr_gen.py``。recipe 设置 ``PYTHONPATH`` 后调用脚本，再 dump 变量并 touch
  stamp，形成可增量复用的 generator build。
* L47-L57：每个 ``$(TESTS-DIR)/%/$(asm-stem)`` 由 ``run_instr_gen.py`` 生成，前置条件是
  generator build、testlist 和脚本本身。``$*`` 是 ``TEST.SEED`` stem，传给脚本后定位单次
  generator 输出目录。

§17.4  ``eh2_sim.mk`` — TB、仿真、覆盖率与结果汇总阶段
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. literalinclude:: ../../../../dv/uvm/core_eh2/scripts/eh2_sim.mk
   :language: make
   :linenos:
   :caption: dv/uvm/core_eh2/scripts/eh2_sim.mk:全文

逐段精读：

* L1-L8：文件头说明该片段提供 RTL testbench 编译、仿真运行、coverage 和 regression
  report 目标，并由顶层 ``wrapper.mk`` include。
* L9-L30：定义 public targets 与 stamp：``rtl_tb_compile``、``rtl_sim_run``、
  ``check_logs``、``riscv_dv_fcov``、``merge_cov`` 和 ``collect_results``。空的
  ``rtl-sim-logs``/``comp-results`` 由 ``wrapper.mk`` 根据 test matrix 追加。
* L31-L38：TB compile 阶段使用 ``.tb.vars.mk`` 记录变量依赖，当前受
  ``SIMULATOR``、``COV`` 和 ``WAVES`` 影响。变量变化时 ``vars-prereq`` 会触发重编译。
* L39-L48：``tb.compile.stamp`` 依赖变量前置条件、Verilog/C++/riscv-dv 文件、
  ``compile_tb.py`` 和 ``rtl_simulation.yaml``。recipe 调用 ``compile_tb.py`` 后 dump
  变量并 touch stamp。
* L50-L60：``rtl-sim-logs`` pattern rule 以已编译 TB、单测 binary 和 ``run_rtl.py`` 为前置，
  对每个 ``TEST.SEED`` 调用 ``run_rtl.py``。这里不直接解析 log，只负责运行 simulator。
* L61-L70：``comp-results`` pattern rule 等待 RTL log 后调用 ``check_logs.py``，输出
  ``trr.yaml``。失败语义在 Python checker 中判定，Make 只维护依赖关系。
* L72-L83：``fcov.stamp`` 在 ``COV=1`` 时调用 ``get_fcov.py`` 生成 riscv-dv functional
  coverage；无 coverage 模式仍 touch stamp，让后续 dependency graph 收敛。
* L85-L96：``merge.cov.stamp`` 在 ``COV=1`` 时调用 ``merge_cov.py`` 合并 coverage 数据；
  同样无条件 touch stamp，避免 non-COV flow 卡住。
* L98-L106：``regr.log.stamp`` 等待 ``collect_results.py``、所有 ``trr.yaml`` 和 coverage
  merge stamp 后运行 ``collect_results.py``，生成 text/JUnit/JSON 报告并 touch stamp。

§18  v2-44 ``env.mk`` 全文行段级精读
--------------------------------------------------------------------------------

``env.mk`` 是仓库根 Make 环境片段，由顶层 ``Makefile`` 通过 ``-include env.mk`` 软加载。
它不是独立命令入口，只提供少量默认值；真正的 compile、regress、sign-off 变量仍以顶层
``Makefile`` 中的同名或相邻变量为准。

.. literalinclude:: ../../../../env.mk
   :language: make
   :linenos:
   :caption: env.mk:全文
...
* L1-L2：文件头说明这是 EH2 UVM verification platform 的 environment makefile，并且被
  main Makefile include。它的职责是提供默认变量，不直接定义 target。
* L4-L5：``EH2_SIMULATOR ?= vcs`` 给 simulator selection 一个默认值。``?=`` 只在变量尚未定义时
  生效，因此命令行或外层环境可以覆盖。
* L7-L8：``WAVES ?= 0`` 默认关闭波形。和 simulator 一样，这只是默认值，后续顶层 target 或
  命令行可以显式设置 ``WAVES=1``。

接口关系：

* 被调用：仓库根 ``Makefile`` 解析阶段通过 ``-include env.mk`` 读取。
* 调用：无 recipe、无子进程。
* 共享状态：只设置 Make 变量 ``EH2_SIMULATOR`` 和 ``WAVES``；不创建文件、不改 shell 环境。
