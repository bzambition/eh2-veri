...
* ``merge_cov.py`` 使用 URG 原生合并 VCS ``.vdb``，不做二次 coverage 后处理。NC 仅用于
  ``make smoke|regress SIMULATOR=nc WAVES=1`` 单测波形调试。
...
.. note::
   ...
   ``SIGNOFF_PROFILE`` 兼容变量，这是历史 Ibex-style wrapper 的内部参数，不应写成
   新用户命令的推荐入口。
...
.. list-table::
   ...
   * - 默认 simulator
     - ``vcs``
     - 与 Ibex 工业 flow 对齐，sign-off/demo 均检查并拒绝非 VCS
   * - coverage 编译维度
   ...
   * - NC 用途
     - ``SIMULATOR=nc WAVES=1`` 单测
     - 不进入 coverage merge、不进入 sign-off 证据
