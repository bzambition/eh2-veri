...
* :ref:`glossary_pretest` — 能区分 smoke、regress、sign-off、:term:`coverage`、
  :term:`waiver` 和 :term:`LEC`；
* :ref:`build_flow` — 知道 ``make signoff`` 会先编译，再由 ``signoff.py`` 调度 stage；
* :ref:`regression_flow` — 知道 ``report.json``、``result.yaml`` 和 ``regr.log`` 的来源；
* :ref:`functional_coverage` — 知道 :term:`LINE` / :term:`GROUP` / :term:`OVERALL` 的区别；
* 基础 Python ``argparse``、JSON、YAML 和返回码概念。

本章讨论的 sign-off（签核）不是“跑一个测试通过”，而是把 smoke、directed、cosim、
riscv-dv、lint、CSR unit、compliance、formal 和 syn/:term:`LEC` 9 个 stage 合并成一个
可审计结论。默认 simulator 是 :term:`VCS`；:term:`NC/Incisive` 是完整备选 simulator，可显式用
...
4. 解释 :term:`LINE` 95.05%、:term:`GROUP` 69.42%、实跑覆盖率 102/104 和
   :term:`LEC` 31635/31635 分别来自哪里。
...
本章的签核事实以 2026-05-19 01:02 :term:`VCS` 主线 demo 为准。该 run 使用
Synopsys :term:`VCS`、:term:`cover.cfg` DUT-only scope、URG 原生 dashboard、block-level
Formality :term:`LEC`，完成 9/9 stages PASS。:term:`NC/Incisive` 是完整备选 simulator，可通过
